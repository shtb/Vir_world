...
void meat_eats::move()
{
      ...
      while(p!=NULL)
      {
            ...
            if(!p->dead)
            {
                  ...
                  else if(p->hungry||p->ener<(p->life))
                  {
                        p->hungry=true;
                        unsigned int a=p->position.x/10;
                        unsigned int b=p->position.z/10;
                        life*disl=my_world.w_grass_eats.live_map[a][b];
                        if(disl==0)
                        {
                              disl=my_world.w_fly_eats.live_map[a][b];
                              if(disl==0&&live_num>10)
                              {
                                    disl=my_world.w_meat_eats.live_map[a][b];
                                    if(disl==p)
                                    {
                                          disl=0;
                                    }
                  ...
                  if(!p->sleep)
                  {
                        ...
                        else
                        {
                              ...
                              float speed=(180-d_angle)/1200.0;
                              if(dv.x>1)
                                    dv.x=1;
                              if(dv.z>1)
                                    dv.z=1;
